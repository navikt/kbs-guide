...
::: {.column-margin}
Merk at [`LangChain` har en god introduksjon til *KBS*][langchain_rag] som
inneholder mer detaljer enn det vi kommer til å gjenskape her.
:::
...
en måte hvor den inneholder dokumentene og så ber modellen svare på påfølgende
spørsmål på bakgrunn av disse dokumentene.
...
Måten denne kjeden fungerer er ved at `context` blir fylt ut av `retriever` (ved
å kalle `__call__` metoden til `retriever` som søker i vektordatabasen med
teksten som kommer inn som argument til hele kjeden) samtidig som `question`
blir fylt ut av teksten som kommer inn som argument til kjeden. Deretter sendes
begge disse over til `prompt` som bruker parameterne (`context` og `question`)
til å formatere ledeteksten. Ferdig utfylt ledetekst blir deretter sendt til
språkmodellen, `llm`, som genererer svaret vårt. Kjeden avsluttes ved å tolke
svaret fra modellen som en ren tekststreng.

Tilslutt, for å benytte kjeden:
