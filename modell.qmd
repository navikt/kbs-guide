...
Denne noden blir inngangen i kjøretidsgrafen vår. Vi kommer tilbake til hva
`state` er litt senere. For nå så kan vi merk oss at i denne noden så benytter
vi oss ikke av en språkmodell. Dette illustrerer styrken i `LangGraph` ved at vi
kan kjede sammen forskjellige noder med forskjellige oppgaver.
...
def context(state):
    ...
    state["rewritten_q"] = context_chain.invoke(
        {
...
def retrieve(state):
    """Node i grafen som henter dokumenter fra vektordatabasen
    basert på spørsmål fra bruker."""
    state["context"] = retriever.invoke(state["rewritten_q"]) # <1>
    return state
```
