...
Vi gjør deretter teksten over om til et `ChatPromptTemplate` som så kan benyttes
av `LangChain`.
...
store = BigQueryVectorSearch.from_documents()
...
> (AI): Samordning mellom dagpenger og sykepenger er relevant når en person er
...
> (AI): Det kan jeg ikke svare på...
```

Når vi ser på dokumentene som ble hentet til det andre spørsmålet til brukeren
så fikk vektordatabasen teksten `Hva innebærer det for pensjon?`. Dette gjør det
veldig vanskelig for vektordatabasen å hente ut informasjon fordi vi manger
kontekst fra det første spørsmålet.

For å utbedre dette kan vi benytte språkmodellen til å omformulere spørsmålet
før vi sender spørsmålet videre til vektordatabasen.
...
chat historikk for å finne dokumenter i vektordatabasen.
...
til å gi kilder. Store språkmodeller som `GPT-4` er gode på å tolke og formulere
både `JSON` og `XML` så dette er et godt utgangspunkt.
