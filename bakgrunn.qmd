...
kunnskapsbase. Dette gir språkmodellen tilgang til ekstra informasjon, man
slipper å dele kunnskapsbasen med tilbyder av språkmodellen samtidig som det har
vist seg å [redusere hallusinering[^2] for språkmodellen][rag_paper]. Samtidig
gjør det at kunnskapsbasen og språkmodellen kan utvikle seg uavhengig av
hverandre og kunnskapsbasen kan oppdateres med ny informasjon uten at
språkmodellen trenger å endre seg. Bakdelen med _KBS_ er at språkmodellen må
kunne klare å hente ut informasjon fra konteksten, noe som kan være utfordrende
med mindre språkmodeller, samtidig som vi bruker plass i kontekstvinduet som
kunne vært brukt til chat-historikk.
...
en [embedding modell][wiki_embedding] (se @def-embedding). Merk at embedding
modellen er uavhengig av språkmodellen i bruk. Ofte brukes en embedding modell
som sammenfaller med språkmodellen, f.eks. brukes OpenAI sin embedding modell
sammen med GPT-4, men det er absolutt ingen krav [^3].
...
::: {.callout-note}
## _KBS_ i NAV
For [NAIS applikasjoner][nais_doc] opprettes det automatisk en Cloud SQL
database. Denne databasen støtter [`pgvector`][pgvector] som kan brukes som et
utgangspunkt for en [vektordatabase][pgvector_gcp].
:::
...
Både [Azure][azure_ai_search] og [Vertex AI][vertex_ai_search] tilbyr egne
databaser for søk sammen med språkmodeller. Argumentet for å bruke disse
databasene sammenlignet med en renere vektordatabase er at både Azure og Vertex
